...
def gaussian_ex(kernel, c):
    sigma = kernel / 4
    s = 2 * (sigma ** 2)
    dx = np.exp(-np.square(np.arange(kernel) - int(kernel * c)) / s)
    return np.reshape(dx, (-1, 1))
    ...
    try:
        ...
        if len(gts) > 0:
            ...
            for ind in range(len(gts)):
                ...
                # # show map
                # dx1 = gaussian_ex(x2 - x1, (c_x - x1) / (x2 - x1))
                # dy1 = gaussian_ex(y2 - y1, (c_y - y1) / (y2 - y1))
                ...
                # scale_map[c_y - r:c_y + r + 1, c_x - r:c_x + r + 1, 2] = np.log(gts[ind, 3] - gts[ind, 1]) / np.log(gts[ind, 2] - gts[ind, 0])
