...
"""
print('*' * 80)
dataset_root = CONFIG.TRAIN.DATA_ROOT
train_dataset = []
...
train_data = DataProviderSet(train_dataset,
                             ...
                             angle_list=CONFIG.TRAIN.ANGLE_LIST)
...
batch_size = len(CONFIG.TRAIN.ANGLE_LIST) * CONFIG.TRAIN.BATCH_SIZE
input_image = tf.placeholder(tf.float32, (batch_size, input_size, input_size, 3), name='input_image')
input_class = tf.placeholder(tf.float32, (batch_size, input_size // down, input_size // down, 1 + 1 + 1), name='input_class')
input_scale = tf.placeholder(tf.float32, (batch_size, input_size // down, input_size // down, 1 + 3), name='input_scale')
input_offset = tf.placeholder(tf.float32, (batch_size, input_size // down, input_size // down, 1 + 2), name='input_offset')
input_landmark = tf.placeholder(tf.float32, (batch_size, input_size // down, input_size // down, 1 + 10), name='input_landmark')

map_class, map_scale, map_offset, map_landmark, map_semantic = net_factory.build(input_image, is_training=True, backbone=CONFIG.MODEL.BACKBONE_NAME)
...
# loss_semantic_op = loss_builder.loss_semantic(input_class, map_semantic)
...
loss_all_op = CONFIG.LOSS.CLASS_WEIGHT * loss_class_op + \
              CONFIG.LOSS.SCALE_WEIGHT * loss_scale_op + \
              CONFIG.LOSS.OFFSET_WEIGHT * loss_offset_op + \
              CONFIG.LOSS.LANDMARK_WEIGHT * loss_landmark_op + \
...
# tf.summary.scalar('loss_semantic', loss_semantic_op)
...
with tf.control_dependencies(update_ops):
    if CONFIG.TRAIN.CONTINUE:
        train_op = tf.train.MomentumOptimizer(learning_rate=CONFIG.TRAIN.LEARNING_RATE, momentum=0.9).minimize(loss_all_op)
    else:
        train_op = tf.train.AdamOptimizer(CONFIG.TRAIN.LEARNING_RATE).minimize(loss_all_op)
...
with tf.Session(config=gpu_config) as sess:
    ...
    try:
        ...
        while True:
            ...
            t2 = time.time()
            global_step += 1
            print('epoch: {: <6d} global_step: {: <6d} train_loss: {: <12.3f} net: {: <12.3f} data: {: <12.3f} debug_info: {}'.format(train_data.curEpoch(),
                                                                                                                                      global_step,
                                                                                                                                      train_loss_,
                                                                                                                                      (t2 - t1) * 1000,
                                                                                                                                      (t1 - t0) * 1000,
